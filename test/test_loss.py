...
from src.loss import (_elementwise_sampled_softmax_loss, approx_epo_loss, binary_cross_entropy_loss, click_ssm_order_be_loss,
                      cosine_loss, sampled_softmax_loss, weighted_loss)
...
def test_weighted_loss():
    click_loss = tensor(4.)
    order_loss = tensor(0.5)
    beta = tensor([[0.6, 0.3]])
    expected_loss = tensor(2.55)

    assert torch.allclose(weighted_loss(click_loss, order_loss, beta), expected_loss)


def test_cosine_loss():
    beta = tensor([[0.5, 0.5]])
    click_elementwise_loss = tensor([[1., -1.], [1., 0.]])
    order_elementwise_loss = tensor([[1., 1.], [1 / 2, 0.]])
    mask = tensor([[1., 1.], [1., 1.]])

    expected_cosine_loss = tensor((-1 - 0. - 0.9486833 - 0.) / 4.)

    assert torch.allclose(cosine_loss(click_elementwise_loss, order_elementwise_loss, mask, beta), expected_cosine_loss)
...
def test_approx_epo():
    ...
    approx_epo = approx_epo_loss(click_elementwise_loss, order_elementwise_loss, mask, beta)
