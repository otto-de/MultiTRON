...
from src.sample import (_infer_shape, _uniform_negatives, _uniform_negatives_session_rejected, sample_in_batch_negatives,
                        sample_uniform, sample_uniform_negatives_with_shape)
...
def test_infer_shape():
    ...
    shape_eventwise = _infer_shape(session_len=session_len,
                                   num_uniform_negatives=num_uniform_negatives,
                                   sampling_style="eventwise")
    shape_sessionwise = _infer_shape(session_len=session_len,
                                     num_uniform_negatives=num_uniform_negatives,
                                     sampling_style="sessionwise")
    shape_batchwise = _infer_shape(session_len=session_len,
                                   num_uniform_negatives=num_uniform_negatives,
                                   sampling_style="batchwise")

    assert shape_eventwise == [5, 2]
    assert shape_sessionwise == [
        2,
    ]
...
def test_sample_uniform_negatives_with_shape():
    ...
    elementwise_negatives = sample_uniform_negatives_with_shape(clicks=clicks,
                                                                num_items=num_items,
                                                                session_len=session_len,
                                                                num_uniform_negatives=num_uniform_negatives,
                                                                sampling_style="eventwise",
                                                                reject_session_items=False)
    sessionwise_negatives = sample_uniform_negatives_with_shape(clicks=clicks,
                                                                num_items=num_items,
                                                                session_len=session_len,
                                                                num_uniform_negatives=num_uniform_negatives,
                                                                sampling_style="sessionwise",
                                                                reject_session_items=False)
    batchwise_negatives = sample_uniform_negatives_with_shape(clicks=clicks,
                                                              num_items=num_items,
                                                              session_len=session_len,
                                                              num_uniform_negatives=num_uniform_negatives,
                                                              sampling_style="batchwise",
...
def test_sample_in_batch_negatives():
    ...
    without_same_session_negatives = sample_in_batch_negatives(batch_positives=batch_positives,
                                                               num_in_batch_negatives=num_in_batch_negatives,
                                                               batch_session_len=batch_session_len,
                                                               reject_session_items=True)
    with_same_session_negatives = sample_in_batch_negatives(batch_positives=batch_positives,
                                                            num_in_batch_negatives=num_in_batch_negatives,
                                                            batch_session_len=batch_session_len,
                                                            reject_session_items=False)
